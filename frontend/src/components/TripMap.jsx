...
export default function TripMap({
    ...
    if (!stops.length) {
      ...
      if (pickupLat && dropoffLat) {
        drawNavLine(L, map,
          ...
          [parseFloat(dropoffLat), parseFloat(dropoffLng)], '#4ade80'
        );
  ...
  // Driver nav line — to next pending pickup stop
  useEffect(() => {
    ...
    if (nextPickup) {
      ...
      setNavInfo({
        dist: dist < 1000 ? `${Math.round(dist)}m` : `${(dist/1000).toFixed(1)}km`,
        target: nextPickup.label || 'Next pickup',
      });
    }
      ...
      <div style={{ background:C.bg3, padding:'10px 14px', display:'flex', gap:16, flexWrap:'wrap', alignItems:'center', borderTop:`1px solid ${C.border}` }}>
        ...
        {navInfo && (
          <span style={{ fontSize:12, color:C.green, fontWeight:500 }}>
            📍 {navInfo.dist} to {navInfo.target}
          </span>
        )}
        {isDriver && (
...
// ── Stop Picker — for Admin to click map and set stops ───
export function StopPicker({ stops, onChange, height = 340 }) {
  ...
  useEffect(() => {
    ...
    stops.forEach((s, i) => {
      ...
      const icon = L.divIcon({
        html: `<div style="background:${color};border:2px solid #fff;border-radius:50%;width:18px;height:18px;box-shadow:0 0 8px ${color}99;display:flex;align-items:center;justify-content:center;font-size:9px;font-weight:700;color:#000">${i+1}</div>`,
        iconSize: [18,18], iconAnchor: [9,9], className: '',
      });
      const m = L.marker([parseFloat(s.lat), parseFloat(s.lng)], { icon })
        .addTo(map)
        .bindPopup(`<b>${s.type === 'pickup' ? '🟢 Pickup' : '🔵 Drop-off'} ${i+1}</b>`);
      markers.current.push(m);
      bounds.push([parseFloat(s.lat), parseFloat(s.lng)]);
    });

    if (bounds.length > 1) {
      const line = L.polyline(bounds, { color:'#4ade80', weight:2, opacity:0.5, dashArray:'6,5' }).addTo(map);
        ...
        <div style={{ background:C.bg3, padding:'10px 14px', borderTop:`1px solid ${C.border}` }}>
          {stops.map((s, i) => (
            <div key={i} style={{ display:'flex', alignItems:'center', gap:8, padding:'4px 0', fontSize:12 }}>
              <span style={{ color: s.type === 'pickup' ? C.green : C.blue }}>{s.type === 'pickup' ? '🟢' : '🔵'} {i+1}.</span>
              <span style={{ color:C.text2 }}>{s.type} — {parseFloat(s.lat).toFixed(4)}, {parseFloat(s.lng).toFixed(4)}</span>
              <input
                value={s.label}
  ...
  useEffect(() => {
    ...
    drivers.forEach(d => {
      ...
      if (pins.current[d.driver_id]) {
        ...
        pins.current[d.driver_id] = L.marker(pos, { icon }).addTo(leafletMap.current)

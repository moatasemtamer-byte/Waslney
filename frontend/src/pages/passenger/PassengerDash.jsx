...
export default function PassengerDash() {
  ...
  // Restore tab from URL hash on load (e.g. #bookings, #history)
  const initTab = () => {
    const h = window.location.hash.replace('#','');
    return ['search','bookings','history'].includes(h) ? h : 'search';
  };
  const [tab, setTab] = useState(initTab);
  const [sub, setSub] = useState(null);
  const [selBooking, setSelBooking] = useState(null); // for booking detail view

  // Sync tab to URL hash
  const changeTab = (t) => {
    setTab(t); setSub(null); setSelBooking(null);
      ...
      <div style={{ maxWidth:860, margin:'0 auto', padding:'28px 20px' }}>
        ...
        {tab === 'search' && sub === 'confirmed' && confirmedBooking && (
          <div style={{ textAlign:'center', padding:'20px 0' }}>
            ...
            <button onClick={() => { changeTab('bookings'); }} style={btnPrimary}>View my bookings</button>
          </div>
        )}
              ...
              return (
                <div key={b.id} onClick={() => setSelBooking(b)}
                  ...
                  <div style={{ display:'flex', alignItems:'center', marginBottom:8 }}>
                    <Badge type={st==='picked'?'green':st==='dropped'?'blue':'amber'}>{statusLabel}</Badge>
                    <span style={{ marginLeft:'auto', fontSize:11, color:C.text3 }}>{fmtDate(b.date)}</span>
                  </div>
                  <div style={{ fontSize:16, fontWeight:500, marginBottom:4 }}>{b.from_loc} → {b.to_loc}</div>
                  <div style={{ fontSize:12, color:C.text2, marginBottom:8 }}>Pickup {b.pickup_time} · {b.driver_name}</div>
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                    <span style={{ fontSize:13, color:C.green, fontWeight:500 }}>{b.seats * b.price} EGP</span>
                    <span style={{ fontSize:12, color:C.text3 }}>Tap to view details →</span>
                  </div>
                </div>
              );
            })}
        ...
        {tab === 'bookings' && selBooking && (() => {
          ...
          return (
            <div>
              ...
              {/* Status banner */}
              {st === 'picked' && (
                <div style={{ padding:'12px 16px', background:C.greenDim, border:'1px solid '+C.greenBorder, borderRadius:10, marginBottom:16, fontSize:13, color:C.green, fontWeight:600 }}>
                  ✅ You've been picked up — driver heading to your drop-off
                </div>
              )}
              {st === 'dropped' && (
                <div style={{ padding:'12px 16px', background:C.blueDim, border:'1px solid '+C.blueBorder, borderRadius:10, marginBottom:16, fontSize:13, color:C.blue, fontWeight:600 }}>
                  🏁 You've been dropped off — thank you for riding!
                </div>
              )}
              ...
              <div style={{ ...card, marginBottom:14 }}>
                ...
                <DetailRow label="Pickup time" val={b.pickup_time} accent={C.green} />
                <div style={{ display:'flex', justifyContent:'space-between', padding:'10px 0' }}>
                  <span style={{ color:C.text2, fontSize:13 }}>Total</span>
                  <span style={{ color:C.green, fontWeight:600 }}>{b.seats * b.price} EGP</span>
                </div>
              </div>
